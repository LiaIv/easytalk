...
EasyTalk — образовательный **монорепозиторий** (FastAPI-бэкенд + iOS-приложение) для детей 8–13 лет. Цель проекта — повысить мотивацию к изучению английского языка через мини-игры, визуальную обратную связь и систему достижений.

## Архитектура монорепозитория

Проект разделён на две взаимосвязанные части:

* **`backend/`** — FastAPI-приложение на Python 3.13, Firebase Admin SDK.
* **`frontend/`** — iOS-приложение на SwiftUI + Combine.

### Backend (слои Clean Architecture)

1. **Domain** — Pydantic-модели (`UserModel`, `SessionModel`, `ProgressItem`).
2. **Repositories** — обёртки Firestore (`UserRepository`, `SessionRepository`).
3. **Services** — бизнес-правила, расчёт достижений.
4. **Routers** — REST-эндпоинты: `/api/profile`, `/api/session`, `/api/progress`, `/api/content`.
5. **Shared** — конфиг и инициализация Firebase (`firebase_client.py`).

OpenAPI-спека генерируется автоматически и лежит в `openapi.json`.

### Frontend (SwiftUI, MVVM)

* SwiftUI + Combine + async/await.
* `AuthService` (Firebase Auth).
* Планируемый пакет `Networking` (`APIClient`, `AuthInterceptor`, `RetryPolicy`).
* UI-модули: AuthView, GameModule, EducationModule, ProfileModule.
...
3. После успешного входа пользователя получите его ID токен. Пример для Swift (iOS):
  ...
  ```swift
    ...
    Этот токен затем используем для отправки запросов к API.
