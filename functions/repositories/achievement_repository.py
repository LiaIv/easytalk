...
class AchievementRepository:
    ...
    def create_achievement(self, achievement: AchievementModel) -> None:
        """
        Сохраняем AchievementModel в Firestore, но поле period_start_date
        преобразуем в ISO‐строку, чтобы Firestore мог принять значение.
        """
        data = achievement.dict()
        # Если period_start_date не None, сохраняем как строку "YYYY-MM-DD"
        if data.get("period_start_date") is not None:
            data["period_start_date"] = data["period_start_date"].isoformat()
