...
# Добавляем директорию backend в sys.path и делаем ее основным пакетом
sys.path.insert(0, str(root_dir))

# Инициализируем все модули как части пакета backend
def init_package():
    # Создаем "виртуальные" пакеты для всех модулей
    for directory in ['domain', 'repositories', 'routers', 'services', 'shared']:
        package_name = f'backend.{directory}'
        module = type(sys)(package_name)
        module.__path__ = [str(root_dir / directory)]
        sys.modules[package_name] = module

    # Устанавливаем текущую директорию как директорию backend
